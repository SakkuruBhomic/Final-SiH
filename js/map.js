...
class DisasterMap {
  ...
  drawRiskZones() {
    ...
    // Render GREEN (outermost / 1.40x) first, down to RED (innermost / 0.35x) last
    // Later = smaller radius drawn last so inner rings visually layer above outer ones like stacked glass plates.
    const CONCENTRIC_TIERS = [
      { level: 'GREEN',  multiplier: 1.40, subLabel: 'LOW RISK',   fillOpacity: 0.16, strokeOpacity: 0.65, ringName: 'Outer Advisory Perimeter' },
      { level: 'YELLOW', multiplier: 1.00, subLabel: 'MODERATE',   fillOpacity: 0.22, strokeOpacity: 0.75, ringName: 'Moderate Monitoring Zone' },
      { level: 'ORANGE', multiplier: 0.65, subLabel: 'HIGH ALERT', fillOpacity: 0.30, strokeOpacity: 0.85, ringName: 'High Alert Buffer' },
      { level: 'RED',    multiplier: 0.35, subLabel: 'CRITICAL',   fillOpacity: 0.38, strokeOpacity: 0.95, ringName: 'Critical Active Core' }
    ];
    ...
    APP_DATA.riskZones.forEach(zone => {
      ...
      if (zone.epicenter || zone.baseRadius) {
        ...
        // Floating centroid label with area name at epicenter
        const labelIcon = L.divIcon({
          html: `
            <div class="zone-floating-label zone-red">
              <span class="zone-label-dot"></span>
        ...
        const labelMarker = L.marker([lat, lng], { icon: labelIcon, interactive: false }).addTo(this.map);
        ...
        const labelIcon = L.divIcon({
          html: `
            <div class="zone-floating-label zone-${zone.level.toLowerCase()}">
              <span class="zone-label-dot"></span>
              <span class="zone-label-name">${zone.name}</span>
            </div>
          `,
        ...
        this.riskZoneCircles.push({ zone, circle: polygonLayer, rings: [polygonLayer], label: labelMarker });
      }
    });
  }
  ...
  addHabitationMarkers() {
    APP_DATA.habitations.forEach(hab => {
      ...
      const marker = L.marker([hab.lat, hab.lng], { icon }).addTo(this.map);
