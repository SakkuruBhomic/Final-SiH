...
async function updateCitizenWeatherAndRisk(lat, lng, place) {
  ...
  // 1. Fetch live current weather from /api/windy/point-forecast with direct Open-Meteo fallback
  ...
  try {
    ...
    // Try browser-direct Open-Meteo fetch
  }

  // Direct client-side Open-Meteo fallback (CORS enabled, 100% resilient without server)
  if (!weatherSuccess) {
    try {
      const omResp = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${Number(lat).toFixed(4)}&longitude=${Number(lng).toFixed(4)}&current=temperature_2m,relative_humidity_2m,precipitation,surface_pressure,wind_speed_10m,wind_gusts_10m&timezone=auto`);
      if (omResp.ok) {
        const omData = await omResp.json();
        const curr = omData.current || {};
        const temp = Math.round(curr.temperature_2m !== undefined ? curr.temperature_2m : 28);
        const wind = Math.round(curr.wind_speed_10m !== undefined ? curr.wind_speed_10m : 14);
        const icon = (curr.precipitation && curr.precipitation > 0.5) ? '🌧️' : (wind >= 50 ? '🌪️' : '🌤️');
...
function selectHazard(key, fly) {
  ...
  // If seismic hazard selected, asynchronously fetch authentic USGS live feed (with direct browser CORS fallback)
  if (key === 'earthquake') {
    const handleQuakeData = (data) => {
    ...
    fetch('/api/earthquakes/live')
      ...
      .then(handleQuakeData)
      .catch(() => {
        // Direct browser USGS query fallback (public CORS-enabled API)
        fetch('https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&minmagnitude=2.5&limit=35')
          .then(r => r.json())
          .then(geojson => {
            ...
            const quakes = features.map(f => {
              ...
              return {
                id: f.id,
                place: p.place || 'Unknown Epicenter',
                mag: p.mag !== null ? Number(p.mag.toFixed(1)) : 0.0,
                time: p.time ? new Date(p.time).toISOString() : new Date().toISOString(),
                epochMs: p.time,
                depthKm: c[2] !== undefined ? Number(c[2].toFixed(1)) : 10,
                lng: c[0],
                lat: c[1],
            ...
            handleQuakeData({
              count: quakes.length,
              maxMagnitude: quakes.length > 0 ? Math.max(...quakes.map(q => q.mag)) : 0,
              latest: quakes[0] || null,
              earthquakes: quakes
            });
          })
          .catch(err => console.warn('Direct USGS live fetch error:', err));
      });
  }
